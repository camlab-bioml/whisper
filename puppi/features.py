...
def feature_engineering(intensity_df: pd.DataFrame, controls) -> pd.DataFrame:
    """
    Feature engineering matching your Benchmarking BioID DIA script, wrapped as a reusable function.

    Parameters
    ----------
    intensity_df : pd.DataFrame
        Must contain a 'Protein' column and intensity columns named like '<BAIT>_<rep>'.
    controls : list of str
        Substrings to identify control columns (e.g., ['EGFP', 'Empty', 'NminiTurbo']).

    Returns
    -------
    pd.DataFrame
        Aggregated engineered features across all baits with the same columns and computations
        as your script, plus composite_score and global_cv, sorted within each bait.
    """
    ...
    # Infer bait names from non-control columns (prefix before first underscore)
    ...
    intensity_columns = all_sample_columns
