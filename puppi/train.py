...
def train_and_score(
    ...
    *,
    ...
    # Cluster only if > 2 baits; else mark all as strong
    if len(bait_names) > 2:
    ...
    # (1) larger membership; (2) tie-break by higher mean bait score
    ...
    # --- Assign positives per bait (fixed cap for strong baits, 0 otherwise) ---
    ...
    # --- Label positives & negatives (exclude single-rep spikes from positives) ---
    ...
    for bait in df_real['Bait'].unique():
        ...
        if N_positives > 0:
            ...
            bottom_negatives = remaining['composite_score'].nsmallest(n_negatives_per_bait).index
    ...
    # --- Scale & train Bagging(RandomForest) ---
    ...
    # --- Global FDR calculation with monotonicity enforcement ---
    unique_real_probs = np.unique(df_real['predicted_probability'])

    # First pass: raw FDR at each probability
    raw_fdr_dict = {}
    for prob in unique_real_probs:
        real_count = int((df_real['predicted_probability'] >= prob).sum())
        decoy_count = int(np.sum(all_decoy_probs_array >= prob))
        raw_fdr = min(decoy_count / real_count if real_count > 0 else 1.0, 1.0)
        raw_fdr_dict[prob] = raw_fdr

    # Second pass: enforce non-increasing FDR as probability increases
    ...
    # --- Flag preys based on global_cv (≤ 25th percentile; matches the code, not the comment) ---
