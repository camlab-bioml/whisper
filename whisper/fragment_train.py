...
def train_and_score_fragment(
    ...
    min_fragments_per_protein: int = 2,
    random_state: int = 42,
    save_dir: str = ".",
    ...
    Parameters of note:
      - n_decoy_replicates: independent feature permutations pooled into the
        ...
        1/sqrt(N) the noise.
      - min_fragments_per_protein: proteins backed by fewer fragments than this
    ...
    # Proteins backed by too few fragments within a bait carry no evidence here.
    if min_fragments_per_protein > 1:
        _before = len(df)
        df = (df.groupby(["Bait", "Protein"])
                .filter(lambda g: len(g) >= min_fragments_per_protein)
                .reset_index(drop=True))
        if len(df) < _before:
            print(f"Dropped {_before - len(df)} row(s) from proteins with "
                  f"fewer than {min_fragments_per_protein} fragments per bait")
